...
var recaptcha = new Recaptcha(config.recaptchaSiteKey, config.recaptchaSecretKey);
...
passport.deserializeUser(function(obj, done) {
  MongoClient.connect(mongodbUrl, function (err, db) {
    db.collection('localUsers').findOne({username:obj.username}, function (err, user) {
      done(err, user);
    });
  });
});
...
.post('/addresses', function(req, res) {
  ensureAuthenticated(req, res, function(req, res) {
    funct.addAddress(req.user.username, req.body.address, req.body.salt, req.body.signature).then(function(a) {
...
app.get('/delete-address', function(req, res) {
  ensureAuthenticated(req, res, function(req, res) {
    funct.deleteAddress(req.user.username, req.param('address')).then(function(a) {
      funct.getAddresses(req.user.username).then(function(a) {
        res.redirect('/addresses');
      });
    });
  ...
  ensureAuthenticated(req, res, function(req, res) {
    if(IsValidThread(req.param('hash'))) {
      if(!req.user.balance) {
        ...
        funct.addComment(req.param('hash'), req.user.username, req.body.comment).then(function(m) {
          res.redirect('/discussion/'+req.param('hash'));
        });
      }
    } else {
